...
import * as productSelector from '../state';
...
export class ProductEditComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.store.pipe(select(productSelector.getCurrentProduct)).subscribe(
      ...
      if (this.productForm.dirty) {
        ...
        if (p.id === 0) {
          this.productService.createProduct(p).subscribe({
            next: product => this.store.dispatch(productAction.SetCurrentProduct(product)),
            error: err => this.errorMessage = err.error
          });
        } else {
